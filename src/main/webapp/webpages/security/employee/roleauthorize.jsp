...
<jsp:include flush="true" page="../../common/default-operation-js.jsp">
	<jsp:param value="${namespace }roleauthorizecommit" name="commiturl"/>
							...
							<ul class="branch in" style="padding-left: 10px; -webkit-padding-start: 10px;">
								<li><input id="all" type="checkbox" /><label for="all">所有角色 all role</label>
									<ul class="branch in">
										...
										<li><input name="roleid" id="${r.id }" value="${r.id }" type="checkbox" /><label for="${r.id }">${r.name } / ${r.code } </label></li>
										</c:forEach>
									</ul>
								</li>
							</ul>
...
$(document).ready(function() {
	...
	if ($("input[name='roleid']").length == ${fn:length(entity.roles) }) {
		$('input').iCheck('check');
	}
	<c:forEach var="e" items="${entity.roles }">$('#${e.id }').iCheck('check');
	</c:forEach>
});
